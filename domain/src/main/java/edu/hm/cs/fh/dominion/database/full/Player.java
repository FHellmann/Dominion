...
public class Player implements WriteablePlayer {
     ...
     * Create a new player.
     *
     * @param changeable of the parent.
    ...
    @Override
    public void pollCards(final int count, final Random random) {
        IntStream.range(0, count)
                .peek(ignore -> {
                    if (getCardDeckPull().isEmpty()) {
                        ...
                        getCardDeckPull().shuffle(random);
                    }
                })
                .forEach(ignore -> getCardDeckPull().poll().ifPresent(card -> getCardDeckHand().add(card)));
    }
}
