...
public final class CheckFactory {
	...
	public static Check hasType(final Card.Type type) {
		return (game, player, card) -> new MoveResult(card.getMetaData().hasType(type),
				"The card does not have the type " + type + ".");
	}
	...
	public static Check hasMoneyToBuyCard() {
		return (game, player, card) -> new MoveResult(
	...
	public static Check isInCardPrice(final int maxPrice) {
		return (game, player, card) -> new MoveResult(card.getMetaData().getCost() <= maxPrice, "The card is to expensive.");
	}
