...
public abstract class AbstractRobotAI extends AbstractRegisteredPlayer {
    ...
    public Move selectMove(final List<Move> moves) {
        ...
        final Optional<Move> victoryToBuy = buyMoves
                ...
                .filter(move -> ((VictoryCard) ((BuyCard) move).getCard().get()).getMetaData().getPoints() > VictoryCard.ESTATE
                        .getMetaData().getPoints()).findFirst();
        ...
        final Optional<Move> treasuryToBuy = buyMoves
                ...
                .filter(move -> ((TreasuryCard) ((BuyCard) move).getCard().get()).getMetaData().getCoints() > TreasuryCard.COPPER
                        .getMetaData().getCoints()).findFirst();
     ...
     *
     * @param allMoves to be possible to play.
     * @return the move or null in an optional.
     */
     ...
     *
     * @param selectedMove which was may selected before.
     * @param optMove      which will be selected if the other one is empty.
     * @return the move.
     */
